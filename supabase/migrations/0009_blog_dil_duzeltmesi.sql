-- ============================================================
-- REHBER — Blog metinlerinin dil ve gramer düzeltmesi (0009)
--
-- Metinler src/content/blog/*.md dosyalarından yeniden üretildi.
-- Düzeltilenler:
--   * "100 gün" yazısındaki "doksan gün" tutarsızlığı,
--   * eksik karşılaştırma ekleri ("uygulamaktan daha kolay", "demekten daha etkili"),
--   * öznesi düşmüş yüklem ("... sonuna kadar taşır"),
--   * anlamı tersine çeviren "çakışıyorsa" ifadesi,
--   * tamlananı eksik "D'lerin ... fazlaysa",
--   * "not düşmek" deyiminin eksik kullanımı,
--   * emir kipindeki başlıkların anlatım kipine çevrilmesi,
--   * düz tırnak/kesme işaretlerinin tipografik karşılıklarıyla değiştirilmesi.
-- ============================================================
...
- **3 gün sonra:** 10 soru. Bu kez farklı tipte sorular.
...
Bu sistemi kurmak, uygulamaktan daha kolay:
...
Denemeyi bitirdiğin gün, en geç ertesi gün yapmak en verimlisi. Kronometreyle şöyle ilerlemek yeterli:
...
D’lerin sayısı toplam yanlışının üçte birinden fazlaysa, önündeki problem konu değil sınav yönetimidir — ve bu, konu çalışmaktan çok daha hızlı düzelir.

> Deneme bir not karnesi değil, teşhis aracıdır. Karne gibi bakınca elde yalnızca bir sayı kalır.
...
update public.posts set icerik = 'Son 100 gün kulağa az geliyor ama aslında **14 tam hafta** demek. Bu haftaları gün gün doldurmaya çalışırsan üçüncü haftada plan çöker. Hafta hafta düşünürsen aynı plan seni sonuna kadar taşır.
...
Yüz günün son on dört gününde yeni konu açılmaz. Bu iki hafta şuna ayrılır:
...
## Yanlış yapmak kadar boş bırakmak da net kaybettirir
...
2. **Çar–Cum:** Kaynağından günde 30 soru. Her yanlışın yanına tek cümlelik bir “neden” notu düşmek faydalı: *kuralı unuttum*, *işlem hatası*, *soruyu yanlış okudum*.
...
Hangi konunun “en çok kaybettiren” olduğundan emin değilsen deneme analizi iyi bir başlangıç: [sınav sonrası 20 dakikalık yöntem](/blog/deneme-analizi-20-dakika) yazısında adım adım anlattık.
...
Basit kural: metin bitmeden seçenek okunmaz.

## 3. Kanıt cümlesini işaretlemek
...
O haftalarda “geriledin mi?” diye sormak, çocuğun tam da doğru şeyi yaptığı bir anda moralini düşürüyor. Koçunuz bu dönemleri önceden söyler; panelde de görürsünüz.
...
- **Sessiz bir saat.** Çalışma saatinde evin ritminin ona göre yavaşlaması, “çalış” demekten daha etkilidir.
- **Sonuç sormadığınız bir gün.** Özellikle deneme günü akşamı. Net konuşulacaksa ertesi gün konuşulur.

Gerisini koç üstleniyor. Size kalan en değerli katkı ise sınav yılını evin tek gündemi olmaktan çıkarmak.
' where slug = 'sinav-yilinda-veli-olmak';

update public.posts set ozet = 'Yanlışları üçe ayırmak yetiyor: bilmiyordum, yanlış okudum, süre yetmedi. Her birinin çözümü farklı.' where slug = 'deneme-analizi-20-dakika';

update public.posts set ozet = 'Gün gün değil, hafta hafta: 1 ana konu, 1 tekrar bloğu, 1 deneme.' where slug = 'lgs-son-100-gun';

update public.posts set ozet = 'Önce soruyu okumak, seçeneğe erken dönmemek, kanıt cümlesini işaretlemek. Hepsi bu.' where slug = 'paragrafta-hiz';

update public.posts set ozet = 'Unutma eğrisiyle savaşmak yerine onu takvime bağlamak: 1 gün, 3 gün, 1 hafta.' where slug = 'aralikli-tekrar';

-- Başlık ve özetlerdeki düz kesme/tırnak işaretleri tipografik karşılıklarıyla
update public.posts set baslik = 'LGS’ye son 100 gün: haftalık ritim nasıl kurulur?' where slug = 'lgs-son-100-gun';
update public.posts set ozet = '“Bugün kaç soru çözdün?” yerine sorulabilecek üç soru.' where slug = 'sinav-yilinda-veli-olmak';
